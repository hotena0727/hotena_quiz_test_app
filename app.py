...
def get_admin_sb():
    """✅ 관리자 전용: Service Role Key 사용 (JWT 만료/RLS 문제 해결)"""
...
def refresh_session_from_cookie_if_needed(force: bool = False) -> bool:
    ...
    - force=False: access_token이 없을 때만 갱신
    - force=True: 무조건 갱신 시도
    """
    if not force and st.session_state.get("access_token") and st.session_state.get("user"):
        return True
...
def get_authed_sb():
    ...
    - 만료 가능성 있음 → DB 호출 시 JWT expired면 refresh 후 rerun
    """
...
def run_with_jwt_retry(fn, *, rerun_on_refresh: bool = True):
    """
    ✅ DB 호출을 감싸서 'JWT expired'면 refresh 후 재시도(또는 rerun)
    """
    try:
        return fn()
    except Exception as e:
        msg = str(e).lower()
        if "jwt expired" in msg or "pgrst303" in msg or "401" in msg:
            ok = refresh_session_from_cookie_if_needed(force=True)
            if ok and rerun_on_refresh:
                st.rerun()
        raise
        ...
        if st.button(
            ...
            try:
                ...
                if "rate limit" in msg and "email" in msg:
                    st.session_state.auth_mode = "login"
                    st.session_state["login_email"] = (email or "").strip()
                    st.session_state.signup_done = False
...
def fetch_all_my_attempts(sb_authed, user_id, limit=500):
    return (
        sb_authed.table("quiz_attempts")
        .select("created_at, level, pos_mode, quiz_len, score, wrong_count")
        .eq("user_id", user_id)
...
def render_admin_dashboard():
    ...
    if st.button("← 퀴즈로 돌아가기", use_container_width=True, key="btn_admin_back"):
        st.session_state.page = "quiz"
        st.rerun()

    sb_admin = get_admin_sb()
    if sb_admin is None:
        st.error("SUPABASE_SERVICE_ROLE_KEY가 설정되지 않았습니다. (Streamlit Secrets 확인)")
        st.stop()

    try:
        res = (
            sb_admin.table("quiz_attempts")
        ...
        st.error("❌ 관리자 조회 실패 (테이블/컬럼/서비스키 확인 필요)")
        st.exception(e)
        st.stop()
...
# ✅ 페이지: 내 기록 대시보드(유저용)
# ============================================================
def render_my_dashboard(user_id: str):
    st.subheader("📈 내 학습 대시보드")

    if st.button("← 퀴즈로 돌아가기", use_container_width=True, key="btn_me_back"):
        st.session_state.page = "quiz"
        st.rerun()

    sb_authed_local = get_authed_sb()
    if sb_authed_local is None:
        st.warning("DB 조회용 토큰이 없습니다. 다시 로그인해 주세요.")
        return

    def _fetch():
        return fetch_all_my_attempts(sb_authed_local, user_id, limit=500)

    try:
        res = run_with_jwt_retry(_fetch)
    except Exception as e:
        st.error("내 기록을 불러오지 못했습니다.")
        st.write(str(e))
        return

    if not res.data:
        st.info("아직 저장된 기록이 없습니다. 퀴즈를 제출하면 기록이 쌓여요.")
        return
    ...
    c1.metric("누적 기록", f"{total}회")
    c2.metric("평균 정답률", f"{avg_rate:.0f}%")
    c3.metric("최고 점수", f"{best} / {int(hist['quiz_len'].max())}")
    c4.metric("최근 점수", f"{last_score} / {last_total}")

    st.divider()

    # 최근 30개 카드
    st.markdown("### 최근 기록(최대 30개)")
    recent = hist.head(30).copy()
...
<div class="record-card">
  <div class="record-top">
    <div>
      <div class="record-title">{badge} {score_i} / {total_i}</div>
      <div class="record-sub">{dt} · {mode} · 레벨 {r["level"]}</div>
    </div>
...
""",
    ...
    st.download_button("⬇️ 내 기록 CSV 다운로드", csv, file_name="my_quiz_attempts.csv", use_container_width=True, key="btn_me_csv")

# ============================================================
# ✅ 페이지: 퀴즈
# ============================================================
def render_quiz(user_id: str, user_email: str):
    # ==========================
    # ✅ CSV 로드
    # ==========================
    ...
    if st.session_state.submitted:
        ...
        if sb_authed_local is None:
            ...
            try:
                res = run_with_jwt_retry(_fetch_recent)
        ...
        if st.button("🗑️ 누적 기록 초기화", use_container_width=True, key="btn_clear_session_stats"):
            st.session_state.history = []
            st.session_state.wrong_counter = {}
            st.session_state.total_counter = {}
            st.rerun()
...
# ✅ RLS용 client + profiles upsert (가능하면)
sb_authed = get_authed_sb()
if sb_authed is not None:
    try:
        run_with_jwt_retry(lambda: ensure_profile(sb_authed, user))
    except Exception:
        pass

# ============================================================
# ✅ 상단 네비(중복 버튼 방지: 여기서만 1번 렌더)
# ============================================================
if "page" not in st.session_state:
    st.session_state.page = "quiz"  # quiz | me | admin
...
with colB:
    if st.button("📝 퀴즈", use_container_width=True, key="nav_quiz"):
        st.session_state.page = "quiz"
        st.rerun()

with colC:
    if st.button("📈 내 기록", use_container_width=True, key="nav_me"):
        st.session_state.page = "me"
        st.rerun()

with colD:
    # 관리자면 '관리자' 버튼 + 로그아웃은 아래에 따로 한 줄
    pass

# 관리자 버튼 + 로그아웃은 한 줄 더 (레이아웃 예쁘게)
colE, colF = st.columns([2, 2])

with colE:
    if is_admin():
        if st.button("📊 관리자", use_container_width=True, key="nav_admin"):
...
if st.session_state.page == "admin":
    ...
    render_my_dashboard(user_id=user_id)
else:
    render_quiz(user_id=user_id, user_email=user_email)
