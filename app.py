...
def clean_candidates(series: pd.Series) -> list[str]:
    """
    보기 후보에서 빈값/'nan' 제거 + 중복 제거
    """
    out = []
...
# ✅ reading/meaning/level/pos 는 필수, jp_word는 비어도 OK
...
def make_question(row: pd.Series, qtype: str, base_pool: pd.DataFrame) -> dict:
    jp_word = norm_text(row.get("jp_word", ""))
    reading = norm_text(row.get("reading", ""))
    meaning = norm_text(row.get("meaning", ""))

    # ✅ 표기 단어: jp_word 없으면 reading으로 대체 (← 네가 원하는 동작)
    ...
    if qtype == "reading":
        ...
        candidates = clean_candidates(
            base_pool.loc[base_pool["reading"] != correct, "reading"]
        )
    else:
        prompt = f"{display_word}의 뜻은?"
        correct = meaning
        candidates = clean_candidates(
            base_pool.loc[base_pool["meaning"] != correct, "meaning"]
        )

    # ✅ 혹시라도 correct가 비어있으면 방지 (데이터 문제)
    if correct == "":
        st.error(f"정답 데이터가 비어있습니다. qtype={qtype}, display={display_word}")
        st.stop()
    ...
    return {
        ...
        "display_word": display_word,  # 화면표시용(절대 nan 아님)
        "reading": reading,
        "meaning": meaning,
        "pos": norm_text(row.get("pos", "")),
        "qtype": qtype,
    }
...
def build_quiz_from_wrongs(wrong_list: list, qtype: str) -> list:
    wrong_words = list({norm_text(w.get("단어", "")) for w in wrong_list})
    wrong_words = [w for w in wrong_words if w]  # 빈값 제거

    # ✅ 오답노트의 '단어'가 display_word(=jp_word 없으면 reading)일 수 있으므로
    # jp_word OR reading 둘 중 하나로 매칭되게 처리
    retry_df = pool_i[
...
if st.session_state.submitted:
    ...
    for idx, q in enumerate(st.session_state.quiz):
        ...
        if picked == correct:
            ...
            wrong_list.append({
                ...
                "내 답": picked,
                "정답": correct,
                # ✅ 오답노트 단어도 display_word로(=jp_word 없으면 reading)
    ...
    for idx, q in enumerate(st.session_state.quiz):
        word_key = q.get("display_word", q.get("jp_word", ""))  # nan 방지
